...
### Error banner for invalid RR group generation
- `manage/page.tsx` now reads `?error=` from `searchParams` and renders a red error banner
- `generateBracketAction` already caught the `assignGroups` throw and redirected with the descriptive message; the page was silently ignoring it

## Version 1.1.5

### Remove BYEs and matches won/lost by default from player match history

### Add Opponent rating (at time of match) to player match history
