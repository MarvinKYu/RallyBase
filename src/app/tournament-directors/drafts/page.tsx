...
export const metadata = { title: "My Drafts — Tournament Directors — RallyBase" };

export default async function TDDraftsPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-semibold text-text-1">Drafts</h1>
          <p className="mt-1 text-sm text-text-2">{tournaments.length} tournament{tournaments.length !== 1 ? "s" : ""}</p>
        </div>

        {tournaments.length === 0 ? (
          <p className="text-sm text-text-2">No draft tournaments.</p>
        ) : (
