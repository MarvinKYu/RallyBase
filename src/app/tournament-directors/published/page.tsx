...
export const metadata = { title: "Published — Tournament Directors — RallyBase" };

export default async function TDPublishedPage() {
  const { userId } = await auth();
  if (!userId) redirect("/sign-in");

  const all = await getMyTournaments(userId);
  const tournaments = all.filter((t) => t.status === "PUBLISHED");

  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-semibold text-text-1">Published</h1>
          <p className="mt-1 text-sm text-text-2">{tournaments.length} tournament{tournaments.length !== 1 ? "s" : ""}</p>
        </div>

        {tournaments.length === 0 ? (
          <p className="text-sm text-text-2">No published tournaments.</p>
        ) : (
