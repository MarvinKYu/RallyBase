...
export function AssignTournamentCreatorForm({
  organizationId,
  action,
}: {
  ...
  return (
    <form action={dispatch} className="flex items-end gap-2">
      ...
      <div className="space-y-1">
        <label htmlFor={`tcPlayerNumber-${organizationId}`} className="block text-xs font-medium text-text-3">
          Add by player #
        </label>
        <input
          id={`tcPlayerNumber-${organizationId}`}
          name="playerNumber"
          type="number"
          min="1"
          placeholder="e.g. 5"
